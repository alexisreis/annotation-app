...
def word_spotting(img_path, x, y, w, h):
    ...
    for l in range(0, len(img) - 1, step):
        for c in range(0, len(img[0]) - 1, step):
            if l not in range(x - step, x + step) and c not in range(y -
                                                                     step, y + step):
                ...
                if score < 125.0:
                    ...
                    # cv.imshow(f'Potentiel{c},{l}', word_test);
                    potentials_words.append([l, c, score])

    # Sort the potential with their score
    potentials_words.sort(key=lambda potential: potential[2])
    ...
    print(f'{potentials_words[0][0]} {potentials_words[0][1]}')

    # return {'x': potentials_words[0][1], 'y': potentials_words[0][2]}
    return potentials_words
